...
- 直接递归检查目录中的视频，按实际容器、编码和像素格式判断兼容性。MP4 / MKV（H.264、8 位 4:2:0、所有音轨均为 AAC/MP3 或无音轨）和 WebM（VP8/VP9、8 位 4:2:0、Opus/Vorbis 或无音轨）会直接跳过，保留原文件，不重新编码或换封装。MKV 的直接播放取决于浏览器支持，不支持时播放器回退 HLS。HEVC 等其他格式仍按现有规则转换。
...
- 进度中显示实际编码器及 GPU/CPU 状态，回退时可展开查看原因。硬件编码使用对应编码器的质量参数，与 CPU 的 `libx264 / medium / CRF 20` 不完全等价，输出大小和指纹可能不同；相同输出指纹仍按原流程去重。当前加速的是视频编码，输入解码、过滤和音频转换仍由 CPU 处理。
...
- 默认使用快速校验：FFmpeg 成功退出后，检查输出为非空普通文件，通过 FFprobe 检查浏览器兼容性、音轨是否缺失和时长差异（允许偏差为 2 秒或源时长的 1%，取较大者）。不再完整解码输出视频，减少额外耗时；快速校验不能检测所有中途损坏。
