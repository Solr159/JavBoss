    ...
    <div className="mx-auto w-full max-w-[1500px] px-4 pt-4 sm:px-6">
      <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">{zh('发现 JAV', 'Discover JAV')}</h1>
            <p className="mt-1 max-w-3xl text-sm text-gray-500">
              {zh(
                '订阅女优后，后台会定时从 JavBus 收集新作品。这里的作品与现有 JAV 资料库完全独立。',
                'Subscribe to idols and the background scanner will collect new works from JavBus. These items remain separate from the main JAV catalog.'
              )}
            </p>
          </div>
          <button
        ...
        <form onSubmit={handleCreate} className="mt-5 grid gap-3 md:grid-cols-[1fr_1fr_auto]">
          ...
          >
            {submitting ? zh('JavBus 校验中…', 'Validating with JavBus...') : zh('添加订阅', 'Add')}
          </button>
        </form>
                ...
                >
                  <div className="min-w-0">
                    ...
                    <div className="truncate text-xs text-gray-500">
                      {subscription.reference_code} · {formatSyncTime(subscription.last_synced_at)}
                    </div>
          ...
          <div className="inline-flex rounded-lg border border-gray-200 bg-white p-1 shadow-sm">
            <button
              ...
              className={`rounded-md px-4 py-2 text-sm font-medium ${
                !wantedOnly ? 'bg-blue-600 text-white' : 'text-gray-600 hover:bg-gray-50'
              }`}
            >
              {zh('已入库', 'Discovered')}
            </button>
            <button
              ...
              className={`rounded-md px-4 py-2 text-sm font-medium ${
                wantedOnly ? 'bg-rose-600 text-white' : 'text-gray-600 hover:bg-gray-50'
              }`}
            >
              {zh('我想要的', 'Wanted')}
            </button>
          </div>
          <span className="text-sm text-gray-500">{zh(`共 ${total} 部`, `${total} items`)}</span>
        </div>
